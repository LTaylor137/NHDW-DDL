...
BEGIN
    ...
    SELECT D.URNUMBER, 'NHRM', 'TABLE', '1', SYSDATETIME(), 'MODIFY'
    FROM @DATA D
    ...
    --         INSERT INTO NHDW_LDT_0214.DBO.ERROR_EVENT
    --     (SOURCE_ID, SOURCE_DATABASE, SOURCE_TABLE, FILTERID, [DATETIME], [ACTION])
    -- SELECT D.URNUMBER, 'NHRM', 'TABLE', '1', SYSDATETIME(), 'SKIP'
    -- FROM @DATA D
    -- WHERE LEN(D.DOB) != 4;
...
BEGIN
    ...
    -- DECLARE @INCORRECT_GENDER_URNUMBERS NVARCHAR(MAX);
    -- SELECT @INCORRECT_GENDER_URNUMBERS = COALESCE(@INCORRECT_GENDER_URNUMBERS + ',', '') + EE.SOURCE_ID
    -- FROM NHDW_LDT_0214.DBO.ERROR_EVENT EE 
    -- WHERE EE.FILTERID = '1' 
    -- AND EE.[ACTION] = 'MODIFY';
    -- -- WHERE D.URNUMBER = (SELECT TOP 1 SOURCE_ID FROM NHDW_LDT_0214.DBO.ERROR_EVENT EE WHERE EE.[ACTION] = 'MODIFY')

    -- PRINT @INCORRECT_GENDER_URNUMBERS;
